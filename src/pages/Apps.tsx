    ...
    <div className="min-h-screen bg-white dark:bg-gray-950 text-black dark:text-gray-100 flex flex-col">
      ...
      <header className="flex items-center justify-between px-3 md:px-6 py-3 md:py-4 border-b border-gray-200 dark:border-gray-800 shrink-0">
        <div className="flex items-center gap-2 md:gap-4">
          <button
            onClick={() => { clearSession(); setStep("input"); setProject(null); setSelectedClipIds([]); setClipEdits({}); setExportedUrls({}); setError(""); }}
        ...
        <div className="flex items-center gap-2 md:gap-3">
          {/* ── Credits badge ── */}
          <CreditsBadge credits={credits} onClick={() => setShowCreditModal(true)} />
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-72 border-r border-gray-200 dark:border-gray-800 overflow-y-auto p-5 hidden lg:flex flex-col shrink-0 bg-gray-50 dark:bg-[#080d1a]">
          {/* Credits panel in sidebar */}
          <div className="rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-4 mb-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Kredit Tersisa</p>
              <button onClick={() => setShowCreditModal(true)}
                className="text-[10px] text-[#1ABC71] font-semibold hover:underline">
                Top Up →
              </button>
            </div>
            <div className={`text-3xl font-black tabular-nums ${credits === 0 ? "text-red-500" : credits <= 3 ? "text-orange-500" : "text-[#1ABC71]"}`}>
              {credits}
            </div>
            <p className="text-[10px] text-gray-400 mt-0.5">
              {credits === 0 ? "⚠️ Habis — top up untuk lanjut" : `1 analisis = 1 credit`}
            </p>
          </div>
          ...
          <div className="space-y-2 mb-6">
            {[
              ...
              <div key={i} className="flex items-center gap-2.5 text-xs">
                ...
                <span className={s.done ? "text-gray-700 dark:text-gray-200" : "text-gray-400"}>{s.label}</span>
              </div>
            ))}
          </div>
        ...
        <main className="flex-1 overflow-y-auto min-w-0 bg-white dark:bg-gray-950">
          <div className="sticky top-0 z-10 flex items-center gap-1 px-3 md:px-6 py-2 md:py-3 bg-white/90 dark:bg-gray-950/90 backdrop-blur border-b border-gray-100 dark:border-gray-800">
            <button onClick={() => setActivePanel("moments")}
              className={`px-3 md:px-4 py-2 rounded-xl text-xs font-medium transition-colors flex items-center gap-1.5 md:gap-2 ${activePanel === "moments" ? "bg-[#1ABC71] text-white" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"}`}>
              <Sparkles size={12} />
            ...
            <button onClick={() => setActivePanel("export")}
              className={`px-3 md:px-4 py-2 rounded-xl text-xs font-medium transition-colors flex items-center gap-1.5 md:gap-2 ${activePanel === "export" ? "bg-[#1ABC71] text-white" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"}`}>
              <Film size={12} />
